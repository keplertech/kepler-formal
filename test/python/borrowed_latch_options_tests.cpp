...
void cppOptions() {
  for (bool sec : {false, true}) {
    for (bool boundaries : {false, true}) {
      ...
      check(defaults.enabled == sec && !defaults.initialInputs && !defaults.initialStorage &&
            !defaults.singleInputChange && !defaults.explicitConfiguration,
            "default-on support invented assumptions or enabled LEC event semantics");
  ...
  for (int field = 0; field < 3; ++field) {
    ...
    const auto partial = options.validated(true, false);
    check(partial.enabled && partial.initialInputs == options.initialInputs &&
          partial.initialStorage == options.initialStorage &&
          partial.singleInputChange == (options.inputChanges == LatchInputChanges::Single) &&
          partial.explicitConfiguration, "optional semantic field was required or defaulted");
  }
  options = contract();
  ...
  options.workers = 0;
  result = options.validated(true, false);
  check(result.enabled && result.explicitConfiguration && !result.singleInputChange &&
        !result.initialInputs && !result.initialStorage && result.workers == 0,
        "resource-only tuning invented initialization");
  options.enabled = false;
  invalid(options, "requires latch_support");
  for (int field = 0; field < 3; ++field) {
    options = contract();
...
void parsed(const std::string& expression, bool success, PyObject* exception = nullptr,
  ...
  if (!accepted) {
    ...
    const auto validated = options.validated(sec, boundaries);
    check(validated.initialInputs == options.initialInputs &&
          validated.initialStorage == options.initialStorage &&
          validated.singleInputChange == (options.inputChanges == LatchInputChanges::Single),
          "parser invented an omitted semantic setting");
  }
}
...
void pythonOptions() {
  ...
  for (const char* key : {"latch_workers", "latch_max_waves", "latch_max_states", "latch_max_transactions",
    ...
    parsed(good + " | {'" + key + "': -1}", false, PyExc_OverflowError);
    parsed("{'" + std::string(key) + "': 1}", true);
    parsed("{'latch_support': False, '" + std::string(key) + "': 1}", false, PyExc_ValueError);
  }
  ...
  parsed("{'latch_input_changes': 'any'}", true);
  parsed("{'latch_initial_inputs': 0}", true);
  parsed("{'latch_initial_storage': 0}", true);
}
}  // namespace
