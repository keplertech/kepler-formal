...
With Nix 2.35+ and `nix-command` / `flakes` enabled, install the CLI on
Intel/AMD Linux or Apple Silicon macOS:

```bash
nix run nixpkgs#cachix -- use keplertech
nix profile add 'git+https://github.com/keplertech/kepler-formal?ref=main&submodules=1#kepler-formal'
kepler-formal --help
```

On NixOS, run `sudo nixos-rebuild switch` after the first command, before
installing. Cached binaries are downloaded; uncached revisions build from source.
